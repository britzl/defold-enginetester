require "cucumber.cucumber"
local wait = require "cucumber.automation.wait"

local function to_quat(x, y, z)
	--[[local c1 = math.cos(math.rad(tonumber(x) / 2))
	local c2 = math.cos(math.rad(tonumber(y) / 2))
	local c3 = math.cos(math.rad(tonumber(z) / 2))
	local s1 = math.sin(math.rad(tonumber(x) / 2))
	local s2 = math.sin(math.rad(tonumber(y) / 2))
	local s3 = math.sin(math.rad(tonumber(z) / 2))
	local qxyz = vmath.quat(
		s1 * c2 * c3 + c1 * s2 * s3,
		c1 * s2 * c3 - s1 * c2 * s3,
		c1 * c2 * s3 + s1 * s2 * c3,
		c1 * c2 * c3 - s1 * s2 * s3
	)--]]

	local qx = vmath.quat_rotation_x(math.rad(tonumber(x)))
	local qy = vmath.quat_rotation_y(math.rad(tonumber(y)))
	local qz = vmath.quat_rotation_z(math.rad(tonumber(z)))

	return qx*qy*qz
end

--[[
vmath.quat(0.42629241943359, 0.19984886050224, 0.59172397851944, 0.65436834096909)
vmath.quat(0.34752306342125, 0.31797593832016, 0.57059419155121, 0.67271196842194)
 } else if ( order === 'XZY' ) {

	 this.x = s1 * c2 * c3 - c1 * s2 * s3;
	 this.y = c1 * s2 * c3 - s1 * c2 * s3;
	 this.z = c1 * c2 * s3 + s1 * s2 * c3;
	 this.w = c1 * c2 * c3 + s1 * s2 * s3;

 }

 return this;

]]
--vmath.quat(0.95154851675034, 0.23929834365845, 0.18930785357952, 0.038134574890137)
--vmath.quat(0.12767943739891, 0.14487813413143, 0.26853582262993, 0.94371432065964)


local function compare_quats(q1, q2)
	if math.abs(q1.x - q2.x) >= 0.00000006 then return false end
	if math.abs(q1.y - q2.y) >= 0.00000006 then return false end
	if math.abs(q1.z - q2.z) >= 0.00000006 then return false end
	if math.abs(q1.w - q2.w) >= 0.00000006 then return false end
	return true
end

Given("game object (.*) is positioned at (.*),(.*),(.*)", function(url, x, y, z)
	wait.switch_context(url)
	go.set_position(vmath.vector3(x, y, z), url)
end)
Given("game object (.*) is rotated (.*),(.*),(.*) degrees", function(url, x, y, z)
	wait.switch_context(url)
	go.set_rotation(to_quat(x, y, z), url)
end)
Given("game object (.*) is scaled to (.*),(.*),(.*)", function(url, x, y, z)
	wait.switch_context(url)
	go.set_scale(vmath.vector3(x, y, z), url)
end)




local function animate(url, property, to, duration, delay, playback)
	wait.switch_context(url)
	go.cancel_animations(url, property)
	go.animate(url, property, playback, to, go.EASING_LINEAR, tonumber(duration), delay)
end

When("I set game object (.*) position to (.*),(.*),(.*)", function(url, x, y, z)
	wait.switch_context(url)
	go.set_position(vmath.vector3(x, y, z), url)
end)
When("I set game object (.*) rotation to (.*),(.*),(.*) degrees", function(url, x, y, z)
	wait.switch_context(url)
	local quat = to_quat(x, y, z)
	go.set_rotation(quat, url)
end)
When("I set game object (.*) scale to (.*),(.*),(.*)", function(url, x, y, z)
	wait.switch_context(url)
	go.set_scale(vmath.vector3(x, y, z), url)
end)
When("I set game object (.*) scale to (.*)", function(url, scale)
	wait.switch_context(url)
	go.set_scale(tonumber(scale), url)
end)
When("I animate game object (.*) (.*) to (.*),(.*),(.*) in (.*) seconds?", function(url, property, x, y, z, duration)
	animate(url, property, vmath.vector3(x, y, z), tonumber(duration), 0, go.PLAYBACK_ONCE_FORWARD)
end)
When("I animate game object (.*) (.*) to (.*),(.*),(.*) in (.*) seconds? (.*)", function(url, property, x, y, z, duration, playback)
	assert(go["PLAYBACK_" .. playback])
	animate(url, property, vmath.vector3(x, y, z), tonumber(duration), 0, go["PLAYBACK_" .. playback])
end)
When("I animate game object (.*) (.*) to (.*) in (.*) seconds?", function(url, property, value, duration)
	animate(url, property, tonumber(value), tonumber(duration), 0, go.PLAYBACK_ONCE_FORWARD)
end)
When("I animate game object (.*) (.*) to (.*) in (.*) seconds? (.*)", function(url, property, value, duration, playback)
	assert(go["PLAYBACK_" .. playback])
	animate(url, property, tonumber(value), tonumber(duration), 0, go["PLAYBACK_" .. playback])
end)
When("I animate game object (.*) rotation (.*),(.*),(.*) degrees in (.*) seconds?", function(url, x, y, z, duration)
	wait.switch_context(url)
	go.cancel_animations(url, "rotation")
	local quat = to_quat(x, y, z)
	go.animate(url, "rotation", go.PLAYBACK_ONCE_FORWARD, quat, go.EASING_LINEAR, tonumber(duration))
end)
When("I cancel the (.*) animation on game object (.*)", function(property, url)
	wait.switch_context(url)
	go.cancel_animations(url, property)
end)
When("I delete game object (.*)", function(url)
	wait.switch_context(url)
	go.delete(url)
end)
When("I set the (.*) property of game object (.*) to hash (.*)", function(property, url, value)
	wait.switch_context(url)
	go.set(url, property, hash(value))
end)
When("I set the (.*) property of game object (.*) to number (.*)", function(property, url, value)
	wait.switch_context(url)
	go.set(url, property, tonumber(value))
end)
When("I set the (.*) property of game object (.*) to url (.*)", function(property, url, value)
	wait.switch_context(url)
	go.set(url, property, msg.url(value))
end)
When("I set the (.*) property of game object (.*) to vector3 (.*),(.*),(.*)", function(property, url, x, y, z)
	wait.switch_context(url)
	go.set(url, property, vmath.vector3(x, y, z))
end)
When("I set the (.*) property of game object (.*) to vector4 (.*),(.*),(.*),(.*)", function(property, url, x, y, z, w)
	wait.switch_context(url)
	go.set(url, property, vmath.vector4(x, y, z, w))
end)
When("I set the game object (.*) as parent of game object (.*)", function(parent_id, child)
	wait.switch_context(child)
	msg.post(child, "set_parent", { parent_id = go.get_id(parent_id), keep_world_transform = 0 })
end)
When("I set the game object (.*) as parent of game object (.*) and keep the world transform", function(parent_id, child_url)
	wait.switch_context(child_url)
	msg.post(child_url, "set_parent", { parent_id = go.get_id(parent_id), keep_world_transform = 1 })
end)



Then("game object (.*) should not be positioned at (.*),(.*),(.*)", function (url, x, y, z)
	wait.switch_context(url)
	assert(go.get_position(url) ~= vmath.vector3(x, y, z), "Did not expect position")
end)
Then("game object (.*) should not be rotated (.*),(.*),(.*) degrees", function (url, x, y, z)
	wait.switch_context(url)
	local quat = to_quat(x, y, z)
	assert(go.get_rotation(url) ~= quat, "Did not expect rotation")
end)
Then("game object (.*) should be positioned at (.*),(.*),(.*)", function (url, x, y, z)
	wait.switch_context(url)
	local expected = vmath.vector3(x, y, z)
	local actual = go.get_position(url)
	assert(actual == expected, ("Expected position %s but got %s"):format(tostring(expected), tostring(actual)))
end)
Then("game object (.*) should be positioned within (%d*) pixels of (.*),(.*),(.*)", function (url, distance, x, y, z)
	wait.switch_context(url)
	local expected = vmath.vector3(x, y, z)
	local actual = go.get_position(url)
	assert(vmath.length(expected - actual) <= tonumber(distance), ("Expected position %s but got %s"):format(tostring(expected), tostring(actual)))
end)
Then("game object (.*) should be world positioned at (.*),(.*),(.*)", function (url, x, y, z)
	wait.switch_context(url)
	local expected = vmath.vector3(x, y, z)
	local actual = go.get_world_position(url)
	assert(actual == expected, ("Expected world position %s but got %s"):format(tostring(expected), tostring(actual)))
end)
Then("game object (.*) should be rotated (.*),(.*),(.*) degrees", function (url, x, y, z)
	wait.switch_context(url)
	local expected = to_quat(x, y, z)
	local actual = go.get_rotation(url)
	assert(compare_quats(actual, expected), ("Expected rotation %s but got %s"):format(tostring(expected), tostring(actual)))
end)
Then("game object (.*) should be world rotated (.*),(.*),(.*) degrees", function (url, x, y, z)
	wait.switch_context(url)
	local expected = to_quat(x, y, z)
	local actual = go.get_world_rotation(url)
	assert(compare_quats(actual, expected), ("Expected world rotation %s but got %s"):format(tostring(expected), tostring(actual)))
end)
Then("game object (.*) should have the (.*) property set to hash (.*)", function(url, property, value)
	wait.switch_context(url)
	local expected = hash(value)
	local actual = go.get(url, property)
	assert(actual == expected, ("Expected %s but got %s"):format(tostring(expected), tostring(actual)))
end)
Then("game object (.*) should have the (.*) property set to number (.*)", function(url, property, value)
	wait.switch_context(url)
	local expected = tonumber(value)
	local actual = go.get(url, property)
	assert(actual == expected, ("Expected %s but got %s"):format(expected, actual))
end)
Then("game object (.*) should have the (.*) property set to url (.*)", function(url, property, value)
	wait.switch_context(url)
	local expected = msg.url(value)
	local actual = go.get(url, property)
	assert(actual == expected, ("Expected %s but got %s"):format(tostring(expected), tostring(actual)))
end)
Then("game object (.*) should have the (.*) property set to vector3 (.*),(.*),(.*)", function(url, property, x, y, z)
	wait.switch_context(url)
	local expected = vmath.vector3(x, y, z)
	local actual = go.get(url, property)
	assert(actual == expected, ("Expected %s but got %s"):format(tostring(expected), tostring(actual)))
end)
Then("game object (.*) should have the (.*) property set to vector4 (.*),(.*),(.*),(.*)", function(url, property, x, y, z, w)
	wait.switch_context(url)
	local expected = vmath.vector4(x, y, z, w)
	local actual = go.get(url, property)
	assert(actual == expected, ("Expected %s but got %s"):format(tostring(expected), tostring(actual)))
end)
Then("game object (.*) should be scaled to (.*),(.*),(.*)", function(url, x, y, z)
	wait.switch_context(url)
	local expected = vmath.vector3(x, y, z)
	local actual = go.get_scale(url)
	assert(actual == expected, ("Expected scale %s but got %s"):format(tostring(expected), tostring(actual)))
end)
Then("game object (.*) should be scaled to (.*)", function(url, scale)
	wait.switch_context(url)
	local expected = tonumber(scale)
	local actual = go.get_scale_uniform(url)
	assert(actual == expected, ("Expected scale %d but got %d"):format(expected, actual))
end)
Then("game object (.*) should be world scaled to (.*)", function(url, scale)
	wait.switch_context(url)
	local expected = tonumber(scale)
	local actual = go.get_world_scale_uniform(url)
	assert(actual == expected, ("Expected world scale %d but got %d"):format(expected, actual))
end)
Then("game object (.*) should be world scaled to (.*),(.*),(.*)", function(url, x, y, z)
	wait.switch_context(url)
	local expected = vmath.vector3(x, y, z)
	local actual = go.get_world_scale(url)
	assert(actual == expected, ("Expected world scale %s but got %s"):format(tostring(expected), tostring(actual)))
end)
